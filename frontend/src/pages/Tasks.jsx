...
function TaskCard({ task, onEdit, onComplete, onArchive, onDelete }) {
  ...
  const dueLabel = task.due_date
    ? isToday(new Date(task.due_date)) ? { text: 'Due today', color: 'text-amber-400' }
      ...
      : isPast(new Date(task.due_date)) && task.status !== 'completed' ? { text: 'Overdue', color: 'text-rose-400' }
        ...
        <button
          ...
          className={`mt-0.5 w-5 h-5 rounded-md border-2 flex items-center justify-center flex-shrink-0 transition-all duration-200
            ${task.status === 'completed'
              ...
              : 'border-bg-border hover:border-primary-500'
            }`}
        >
          {task.status === 'completed' && (
            ...
            <div className="relative flex-shrink-0">
              ...
              {menuOpen && (
                <>
                  ...
                  <div className="absolute right-0 top-8 z-20 bg-bg-elevated border border-bg-border rounded-xl shadow-xl py-1.5 min-w-[140px]">
...
export default function Tasks() {
  ...
  const handleArchive = async (id) => {
    await tasksService.archiveTask(id)
    fetchTasks()
  }
