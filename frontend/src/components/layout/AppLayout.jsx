...
function SunIcon()  { return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8} className="w-4 h-4"><circle cx="12" cy="12" r="4"/><path d="M12 2v2M12 20v2M4.93 4.93l1.41 1.41M17.66 17.66l1.41 1.41M2 12h2M20 12h2M4.93 19.07l1.41-1.41M17.66 6.34l1.41-1.41"/></svg> }
function MoonIcon() { return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8} className="w-4 h-4"><path d="M21 12.79A9 9 0 1111.21 3 7 7 0 0021 12.79z"/></svg> }
...
const USER_NAV = [
  ...
  {
    to: '/feedback', label: 'My Feedback',
    icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8} className="w-5 h-5"><polygon points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26"/></svg>,
  },
]
...
export default function AppLayout() {
  ...
  return (
    <div className="min-h-screen bg-bg-base mesh-bg flex transition-colors duration-300">
      ...
      {/* Sidebar */}
      <aside className={`fixed lg:sticky top-0 h-screen w-64 bg-bg-surface border-r border-bg-border z-30 flex flex-col transition-all duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        ...
        <div className="p-6 border-b border-bg-border flex items-center justify-between">
          ...
          <button onClick={toggleTheme} title={isDark ? 'Light mode' : 'Dark mode'}
            className={`w-9 h-9 rounded-xl border border-bg-border flex items-center justify-center transition-all duration-200 hover:border-primary-500/40 ${isDark ? 'bg-bg-elevated text-amber-400 hover:bg-amber-500/10' : 'bg-bg-elevated text-primary-400 hover:bg-primary-500/10'}`}>
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          ...
          {/* Admin — only for is_staff */}
          {user?.is_staff && (
            <>
              <div className="pt-3 pb-1">
                <p className="text-xs font-medium text-text-muted uppercase tracking-wider px-3">Admin</p>
              </div>
              <NavLink to="/admin" onClick={() => setSidebarOpen(false)}
                className={({ isActive }) =>
                  `flex items-center gap-3 px-3 py-2.5 rounded-xl font-medium text-sm transition-all duration-200
                  ${isActive ? 'bg-rose-500/10 text-rose-400 border border-rose-500/20' : 'text-text-secondary hover:text-text-primary hover:bg-bg-elevated'}`
                }>
          ...
          <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-bg-elevated">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-text-muted truncate">
                {user?.is_staff ? '👑 Admin' : user?.email}
              </p>
            </div>
            <button onClick={handleLogout} className="text-text-muted hover:text-rose-400 transition-colors" title="Logout">
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        <header className="lg:hidden sticky top-0 z-10 bg-bg-surface/80 backdrop-blur-md border-b border-bg-border px-4 py-3 flex items-center justify-between">
          ...
          <button onClick={toggleTheme} className={`w-9 h-9 rounded-xl border border-bg-border flex items-center justify-center transition-all duration-200 ${isDark ? 'bg-bg-elevated text-amber-400' : 'bg-bg-elevated text-primary-400'}`}>
            {isDark ? <SunIcon/> : <MoonIcon/>}
          </button>
        </header>
        <main className="flex-1 p-6 lg:p-8 animate-fade-in"><Outlet/></main>
      </div>
    </div>
  )
}
